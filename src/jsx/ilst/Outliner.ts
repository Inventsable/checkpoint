...
import { rgbColor, cmykColor, ColorValue } from "../../shared/shared";
...
function startOutliner(data: string) {
...
function convertListToOutlines(list) {
  for (var i = list.length - 1; i >= 0; i--) {
    ...
    item.name = renameGenericPaths
      ? rollName(parentage, item, item.layer)
      : parentage;
...
function drawAnchor(point, layer, name, group) {
  const root = groupRelated ? group : app.activeDocument;
